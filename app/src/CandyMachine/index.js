...
const CandyMachine = ({ walletAddress }) => {
  ...
  const [mintMessage, setMintMessage] = useState("Mint NFT");
  ...
  const mintToken = async () => {
    ...
    try {
      ...
      if (response) {
        setMinting(false);
        setMintMessage("Mint NFT");
        setState({ ...state, open: true, message: "Congrats! Mint successfull" });
      }
      return response.txs.map((t) => t.txid);
    } catch (e) {
      setMinting(false);
      setMintMessage("Mint NFT");
      setState({ ...state, open: true, message: "Mint failed" });
      console.log(e);
    }
    return [];
  };
        ...
        <div className="machine-container">
          ...
          candyMachine.state.itemsAvailable ? (
            ...
            <button
              ...
              onClick={mintToken}
              disabled={
                currentDate < new Date(candyMachine.state.goLiveData * 1000)
              }
            >
              { minting &&  <Spinner size="1rem" /> }
             {mintMessage}
            </button>
          )}
        </div>
